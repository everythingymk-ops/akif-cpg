...
| 23 | Cloud migration: the pre-cloud product Yahya built on 2026-08-11 ("Godiva Domes Dark Chocolate 12-piece") moved into Supabase with ids preserved; fixed two foreign-key write races that had already cost the seeded Godiva scenario in production | done (2026-08-13) |
...
- **Scenarios carry a foreign key to their product, so writes are ordered, not concurrent.** `Promise.all([upsertProduct(p), upsertScenario(s)])` loses that race intermittently and Postgres rejects the scenario — which is exactly how the shipped Godiva example reached production with its product but no scenario. Always chain: product first, then its scenarios (`product-provider.tsx`, both `addProduct` and the seed path).
